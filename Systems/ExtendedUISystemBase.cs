...
namespace EXRScreenshot.Systems
{
	...
	{
		private const string ModID = "EXRScreenshot";
		...
		public ValueBindingHelper<T> CreateBinding<T>(string key, string setterKey, T initialValue, Action<T> updateCallBack = null)
		{
			...
			var trigger = new TriggerBinding<T>(ModID, setterKey, helper.UpdateCallback, initialValue is Enum ? new EnumReader<T>() : null);
		...
		{
			var binding = new TriggerBinding<T1>(ModID, key, action);
		...
		{
			var binding = new TriggerBinding<T1, T2>(ModID, key, action);
		...
		{
			var binding = new TriggerBinding<T1, T2, T3>(ModID, key, action);
		...
		{
			var binding = new TriggerBinding<T1, T2, T3, T4>(ModID, key, action);
